...
void *CreateVertSlidingDoor(struct Object *arg0, u8 arg1) {
    ...
    ObjectSetHitbox(&door->obj2.base, -8, -0x20, 8, 0);
...
static void VertSlidingDoorOpen(struct VertSlidingDoor *door) {
    ...
    if (door->unkB6 > 6) {
        ...
        if (door->unkB7 <= 7) {
            ...
            ObjectSetHitbox(&obj->base, -8, -0x20, 8, -(door->unkB7 * 4));
        }
    }
    door->unkB6++;
    if (obj->base.flags & 2) {
...
static void VertSlidingDoorClose(struct VertSlidingDoor *door) {
    ...
    if (obj->base.flags & 2) {
        ...
        ObjectSetHitbox(&obj->base, -8, -0x20, 8, 0);
        obj->unk78 = VertSlidingDoorIdle;
        door->unkB4 &= ~1;
        door->unkB4 &= ~2;
    }
...
void VertSlidingDoorSetFunc(struct VertSlidingDoor *arg0) {
    struct VertSlidingDoor *door = arg0;
