...
void *Create8DirCannon(struct Object *arg0, u8 arg1) {
    ...
    ObjectSetHitbox(&cannon->obj2.base, -8, -8, 8, 8);
...
static void EightDirCannonIdle(struct EightDirCannon *cannon) {
    ...
    if (cannon->unkB7 & 8 && cannon->obj2.base.flags & 0x40000) {
        if (((struct ObjectBase *)cannon->obj2.base.unk6C)->unk56 >= gNumPlayers)
            return;
        cannon->unkB7 |= 1;
        ...
        if (gKirbys[0].base.base.base.roomId == roomId && gNumPlayers > 0
            && !(cannon->unkBA & 1)
        ...
        if (gKirbys[1].base.base.base.roomId == roomId && gNumPlayers > 1
            && !(cannon2->unkBA & 2)
        ...
        if (gKirbys[2].base.base.base.roomId == roomId && gNumPlayers > 2
            && !(cannon2->unkBA & 4)
        ...
        if (gKirbys[3].base.base.base.roomId == roomId && gNumPlayers > 3
            && !(cannon2->unkBA & 8)
...
#ifndef NONMATCHING
NAKED static void EightDirCannonRotate(struct EightDirCannon *cannon) {
    asm(".include \"asm/nonmatching/EightDirCannonRotate.inc\"");
}
#else
static void EightDirCannonRotate(struct EightDirCannon *cannon) {
    ...
    if (cannon->unkB7 & 1) {
        if (!(cannon->obj2.object->unk22 & 1)) {
            if (cannon->unkBC->unk11A & 1) {
                for (i = 0; i < gNumKirbys; i++) {
                    if ((cannon2->unkBA >> i) & 1)
                        sub_08051F70(&gKirbys[i], cannon->unkB4);
                }
                cannon->obj2.unk78 = EightDirCannonRecoil;
                cannon->unkB7 &= ~8;
                sub_081222AC(&cannon->obj2.base, cannon->unkB4);
                RequestScreenShake(3, &cannon->obj2.base);
            }
        }
        else if (cannon->unkB7 & 2 && cannon->unkB4 == cannon->obj2.object->unk14) {
            for (i = 0; i < gNumKirbys; i++) {
                if ((cannon2->unkBA >> i) & 1)
                    sub_08051F70(&gKirbys[i], cannon->unkB4);
            }
